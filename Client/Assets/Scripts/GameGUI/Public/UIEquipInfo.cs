...
public class UIEquipInfo : GTWindow
{
    ...
    public void ShowViewByPos(EPosType posType, int pos)
    {
        ...
        int itemID = equip.Id;
        ShowBaseView(itemID);
        equipFightValue.text = GTTools.Format("Força de Combate {0}", GTAttrHelper.GetFightValue(equip));
        ...
        title3.text = GTTools.Format("Estrelas do Equipamento {0}", equip.StarLevel);
        title2.text = GTTools.Format("Nível de Avanço {0}", equip.AdvanceLevel);
        title1.text = GTTools.Format("Nível de Fortalecimento {0}", equip.StrengthenLevel);
