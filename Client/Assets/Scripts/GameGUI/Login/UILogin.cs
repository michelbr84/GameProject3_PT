...
public class UILogin : GTWindow
{
    ...
    {
        versionNumber.text = GTTools.Format("Versão：{0}", Application.version);
    ...
    public void ShowCurrServer()
    {
        ...
        curServerName.text = data == null ? "Servidor" : data.Name;
    }

    public void ShowUsernameAndPassword()
    {
        string username = MLLogin.Instance.LastUsername;
        curAccount.text = string.IsNullOrEmpty(username) ? "Conta" : username;
    }
}
